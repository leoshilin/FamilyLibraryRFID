...
- **API 入参和返回值统一**为 camelCase，并跨接口一致：`itemId` / `familyId` / `bookshelfId` / `bookMetaId`、`createdAt` / `createdBy`、`sortOrder`、`coverUrl`、`bookshelfName`、 `isSet` 等
- **前端实体（miniprogram）**：发送与读取均不再使用 snake_case。
