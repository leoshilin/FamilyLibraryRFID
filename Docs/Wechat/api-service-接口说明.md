...
G3| api_task_getBindStatus | task | 手机端操作：绑定状态查询|
...
2. `services/` 是**唯一**允许 `wx.cloud.callFunction()` 的位置（对应代码审查发现的 20 处违规，整改后即消除）。
...
按业务领域将 33 个 `api_*` 归入 7 个 Service 模块，文件位于 `miniprogram/services/`：
 ...
 Service 文件 | 领域 | 封装的 API |
---|---|---|
 `userServices.js` | 用户 | A1–A4|
 `familyServices.js` | 家庭 | B1–B6|
 `bookshelfServices.js` | 书架 | C1–C5|
 `bookMetaServices.js` | 书本主数据 | D1–D2|
 `bookItemServices.js` | 实体书上下架 | E1–E7|
 `bookSearchServices.js` | 检索 / 最近 | F1–F2|
 `taskServices.js` | 任务 / RFID | G1 / G2 / H1（手机端）+ J1–J4（PDA）|
...
> 桩函数（🚧）对应的 Service 方法随云函数实现一并提供，无需再改页面 / Service 接口。

## 2.4 Service 方法总览（33 个 API → 方法映射）

 编号 | API | Service 模块 | Service 方法 | 关键入参
---|---|---|---|---|
...
```
> 现有 `userServices.js` 实现 `login` / `register` / `updateUser`，需补 `getUser`（对应 A3 桩函数）。
...
```
> 实现 `list` / `create` / `update` / `remove` / `reorder`；`reorder` 对应 C5，由 mine 页「↑ / ↓」按钮调用以重排书架。

### 2.5.4 bookMetaServices.js（书本主数据）
```js
getByIsbn(isbn)                           // → api_bookmeta_getByIsbn     { isbn }
fetchExternal(isbn)                       // → api_bookmeta_fetchExternal { isbn }
```

### 2.5.5 bookItemServices.js（实体书上下架）
```js
...
### 2.5.6 bookSearchServices.js（检索 / 最近）
```js
...
- 桩函数（🚧）对应的 Service 方法随云函数实现一并提供，页面 / Service 契约保持稳定。
...
> **说明**：`api_bookshelf_reorder` 实现真实排序逻辑（见 `cloudfunctions/api_bookshelf_reorder/index.js`）；`bookshelfServices.reorder()` 封装；mine 页「↑ / ↓」按钮点击后交换本地顺序并调用 `reorder` 持久化。
...
{ "success": false, "message": "<err.message>" }
```
> 说明：统一为 `success` 标志（见 0.4 通用约定、第 5 章错误枚举）。`exists` 仅用于区分"系统已存在 / 不存在"两种**成功**结果（不存在不是错误，是转外部源抓取的正常前置条件），因此仍随 `success: true` 一并返回。原文档的 `{ "exists": false }`（无 `success`）与 D2 的 `{ "success": ... }` 不一致，本节统一其返回结构——见遗留问题 6。
...
```
> familyId 与 operator 由服务端从登录态解析（见 0.4），**不接收客户端传入**。原文档入参中的 `familyId`、`operator` 不再作为入参（由服务端解析，见 0.4）。
...
```
> familyId 与 operator 由服务端从登录态解析（见 0.4），**不接收客户端传入**。原文档入参中的 `family_id`、`operator` 不再作为入参（由服务端解析，见 0.4）。
> 注：本接口入参字段统一为 camelCase `itemId`（与 api_bookitem_get / updateBookshelf 一致，见 0.2 命名统一专项）。
...
> 需研究：该函数被 `async loadFromId(itemId)` 调用，但 `loadFromId` 没有被任何地方调用。接口实现但当前无调用方，<font color="red">待确认</font>是否保留或接入页面。
...
```
> 注：本接口入参字段为 camelCase `itemId`，与 E3/E4/E5 统一（见 0.2 命名统一专项）。
...
> **说明**：`api_task_createBindRfid` 实现真实创建逻辑（写入 `device_task`，状态 `pending`）。
...
> **说明**：`api_task_createFindBook` 实现真实创建逻辑（写入 `device_task`，`target_tid` 取图书已绑标签；未绑 RFID 拒绝）。
...
当前版本虽然业务中未出现入口，但未来（图书详情 → 解绑 RFID）大概率会需要，建议现在预留 <font color="red">（待实现）</font>。

> **说明**：`api_task_unbindRfid` 实现真实解绑逻辑（事务清空 `rfid_tid` + 写 `rfid_bind_log`）。
> 注：架构表原用名 `api_rfid_unbind`，与代码实现名 `api_task_unbindRfid` 对齐。
...
```
> operator 由服务端从登录态解析，不接收客户端传入。原文档入参写作 `{ "bookItemId", "operator"" }`（非法 JSON），修正为合法对象。
...
> **说明**：`api_task_complete` 实现真实提交逻辑（更新任务最终状态 / 结果）。
...
> **说明**：`api_task_getRfidBindingInfo` 实现真实查询逻辑（按 `rfid_tid` 反查占用图书）。
> 注：架构表原用名 `api_rfid_getBindingInfo`，与代码实现名 `api_task_getRfidBindingInfo` 对齐。
...
> **说明**：`api_task_bindRfid` 实现真实绑定逻辑（事务处理 4 种绑定场景 A/B/C/D + 写 `rfid_bind_log`）。
> 注：架构表原用名 `api_rfid_bind`，与代码实现名 `api_task_bindRfid` 对齐。
...
> **现状（待系统化，即遗留问题 5.2）**：当前真实云函数在 `checkPermission` 返回 `allowed: false` 时，仅透传 `perm.message`（`return { success: false, message: perm.message }`），**未将 `reason` 透出到接口响应**。因此 `reason` 目前是"代码内部实现、接口层面未暴露"的状态。建议后续在失败响应中统一补充 `reason` 字段（<font color="red">待实现</font>）：
> ```json
...
- **业务规则拒绝**：如"书架下存在在架图书，无法删除""当前家庭已存在有效书架"等，返回 `{ success: false, message: "<业务提示>" }`；如需细分，可补充业务级 `reason`（建议后续扩展，如 `SHELF_NOT_EMPTY` 等）<font color="red">（待实现）</font>。
...
1. **通用失败返回 / 错误枚举未系统化**：权限校验经 `checkPermission` 统一产出 `reason` 错误枚举（见 `_shared/permission.js`），但当前真实云函数仅把 `message` 透传给前端、**未透传 `reason`**（见各接口"返回-失败"示例）。第 5 章《通用错误返回规范（错误枚举）》集中说明推荐结构与枚举表；后续统一在失败响应中补充 `reason` 字段（<font color="red">待实现</font>），使前端可按错误类型分支。
